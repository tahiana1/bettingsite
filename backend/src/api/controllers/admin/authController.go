...
import (
	...
	"github.com/hotbrainy/go-betting/backend/api/controllers/casinoController"
...
func SignUp(c *gin.Context) {
	...
	userExists, err := casinoController.checkUserExists(userInput.Userid)
	if err != nil {
		// Log error but don't fail the signup process
		fmt.Printf("Error checking Honorlink user: %v\n", err)
	} else if !userExists {
		// User doesn't exist, create it
		if err := casinoController.createUser(userInput.Userid); err != nil {
			// Log error but don't fail the signup process
