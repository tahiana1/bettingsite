...
const Notice: React.FC<{checkoutModal: (modal: string) => void}> = (props) => {
    ...
    const expandedRowRender = (record: any) => {
        return (
            <div className="p-4 bg-[#160d0c]">
                ...
                <p className="text-white">{record.description && <div dangerouslySetInnerHTML={{ __html: record.description }} />}</p>
            </div>
        );
    };
        ...
        <Layout.Content className="w-full border-1 bg-[#160d0c] border-[#3e2e23] notice-section">
            <Card
                title={
                    <div className="relative">
                        <h2 className="text-[#edd497] text-[40px] justify-center flex pt-10 font-bold">{t("notice")}</h2>
            ...
            >
                <div className="flex w-full mb-6 bg-gradient-to-r from-[#2a1810] to-[#3e2a1f] rounded-lg overflow-hidden border border-[#5d4a3a]">
                    <button
                        onClick={() => { setSelectedItem(null); props.checkoutModal("deposit") }}
                    ...
                    <button
                        className="flex-1 flex items-center justify-center gap-2 cursor-pointer text-[15px] px-4 py-3 text-white hover:bg-[#2a1810] transition-colors border-r border-[#5d4a3a]"
                        onClick={() => { setSelectedItem(null); props.checkoutModal("withdraw") }}
                    >
                    ...
                    <button 
                        ...
                        onClick={() => { setSelectedItem(null); props.checkoutModal("event") }}
                    >
                        <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                ...
                {!selectedItem ? (
                    <Table
                        ...
                        pagination={{
                            ...
                            total: data?.response?.announcements?.length || 0,
                    ...
                    <div className="px-4 pt-4 pb-10 bg-[#160d0c]">
                        ...
                        <p className="text-white">{selectedItem.description && <div dangerouslySetInnerHTML={{ __html: selectedItem.description }} />}</p>
                    </div>
                )}
            </Card>
        </Layout.Content>
