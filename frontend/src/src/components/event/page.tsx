        ...
        <Layout.Content className="w-full border-1 bg-[#160d0c] border-[#3e2e23] event-section">
            ...
            >
                <div className="flex w-full mb-6 bg-gradient-to-r from-[#2a1810] to-[#3e2a1f] rounded-lg overflow-hidden border border-[#5d4a3a]">
                    <button
                        onClick={() => { setSelectedItem(null); props.checkoutModal && props.checkoutModal("deposit") }}
                    ...
                    <button
                        className="flex-1 flex items-center justify-center gap-2 cursor-pointer text-[15px] px-4 py-3 text-white hover:bg-[#2a1810] transition-colors border-r border-[#5d4a3a]"
                        onClick={() => { setSelectedItem(null); props.checkoutModal && props.checkoutModal("withdraw") }}
                    >
                    ...
                    <button 
                        className="flex-1 flex items-center justify-center gap-2 cursor-pointer text-[15px] px-4 py-3 text-white hover:bg-[#2a1810] transition-colors border-r border-[#5d4a3a]"
                        onClick={() => { setSelectedItem(null); props.checkoutModal && props.checkoutModal("point") }}
                    >
                    ...
                    <button 
                        className="flex-1 flex items-center justify-center gap-2 cursor-pointer text-[15px] px-4 py-3 text-white hover:bg-[#2a1810] transition-colors border-r border-[#5d4a3a]"
                        onClick={() => { setSelectedItem(null);props.checkoutModal && props.checkoutModal("rolling")}}
                    >
                        <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                    ...
                    <button 
                        className="flex-1 flex items-center justify-center gap-2 cursor-pointer text-[15px] px-4 py-3 text-white hover:bg-[#2a1810] transition-colors border-r border-[#5d4a3a]"
                        onClick={() => { setSelectedItem(null); props.checkoutModal && props.checkoutModal("notice") }}
                    >
                        <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                    ...
                    <button 
                        ...
                        onClick={() => { setSelectedItem(null); props.checkoutModal && props.checkoutModal("event") }}
                    >
                        <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
            ...
            {!selectedItem ? (
                ...
                <div className="px-4 pt-4 pb-10 bg-[#160d0c]">
                    ...
                    <div className="text-white">{selectedItem.description && <div dangerouslySetInnerHTML={{ __html: selectedItem.description }} />}</div>
                </div>
            )}
        </Layout.Content>
    );
