...
export default function DhpowerballAdminPage() {
    ...
    const handleDeleteOption = async (optionId: string | number, category: 'powerball' | 'normalball') => {
        if (!confirm(t('dhpowerballadmin/confirmDelete'))) return;
    ...
    return (
        <Layout>
            <Card title={t("dhpowerballPowerballAdminManagement")}>
                {loading && (
                    ...
                    <>
                        <div className="level-buttons-container">
                            {Array.from({ length: 15 }, (_, index) => {
                                ...
                                if (level <= 12) {
                                    levelName = `${t('dhpowerballadmin/level')} ${level}`;
                                } else if (level === 13) {
                                    levelName = 'VIP 1';
                                } else if (level === 14) {
                                    levelName = 'VIP 2';
                        ...
                        <div className="max-betting-container" style={{ marginTop: 16, marginBottom: 16 }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                <label style={{ color: '#f3f4f6', fontWeight: 500 }}>
                                    {t('dhpowerballadmin/maxBettingValue')}:
                                </label>
                                <Input
                                    type="number"
                                    value={maxBettingValue}
                                    onChange={(e) => setMaxBettingValue(e.target.value)}
                                    placeholder={t('dhpowerballadmin/placeholderMaxBetting')}
                                    style={{ width: 120 }}
                                    size="small"
                                />
                                <Button
                                        ...
                                        <div className="bet-sidebar">
                                            <div className="pick-wrap">
                                                <div className="pick-header">
                                                    <span>{t('dhpowerballadmin/powerballCombinations')}</span>
                                                    <div className="flex items-center gap-2">
                                                        <button 
                                                            className="px-2 py-1 bg-blue-600 text-white text-xs rounded hover:bg-blue-700"
                                                            onClick={() => handleAddOption('powerball')}
                                                        >
                                                            + {t('dhpowerballadmin/add')}
                                                        </button>
                                                        <i 
                                            ...
                                            <div className="pick-wrap">
                                                <div className="pick-header">
                                                    <span>{t('dhpowerballadmin/normalballCombinations')}</span>
                                                    <div className="flex items-center gap-2">
                                                        <button 
                                                            className="px-2 py-1 bg-blue-600 text-white text-xs rounded hover:bg-blue-700"
                                ...
                                <div className="modal-overlay">
                                    <div className="modal-content">
                                        <div className="modal-header">
                                            <h3>{editingOption ? t('dhpowerballadmin/editBettingOption') : t('dhpowerballadmin/addNewBettingOption')}</h3>
                                            <button 
                                        ...
                                        <div className="modal-body">
                                            <div className="form-group">
                                                <label>{t('dhpowerballadmin/optionName')}</label>
                                                <input 
                                                    type="text"
                                                    value={formData.name}
                                                    onChange={(e) => setFormData({...formData, name: e.target.value})}
                                                    placeholder={t('dhpowerballadmin/placeholderOptionName')}
                                                />
                                            </div>
                                            
                                            <div className="form-group">
                                                <label>{t('dhpowerballadmin/odds')}</label>
                                                <input 
                                                    type="number"
                                                    step="0.01"
                                                    value={formData.odds}
                                                    onChange={(e) => setFormData({...formData, odds: e.target.value})}
                                                    placeholder={t('dhpowerballadmin/placeholderOdds')}
                                                />
                                            </div>
                                            
                                            <div className="form-group">
                                                <label>{t('dhpowerballadmin/type')}</label>
                                                <select 
                                                    value={formData.type}
                                                    onChange={(e) => setFormData({...formData, type: e.target.value as 'single' | 'combination'})}
                                                >
                                                    <option value="single">{t('dhpowerballadmin/singleBall')}</option>
                                                    <option value="combination">{t('dhpowerballadmin/combination')}</option>
                                                </select>
                                            </div>
                                            ...
                                            {formData.type === 'single' ? (
                                                <>
                                                    <div className="form-group">
                                                        <label>{t('dhpowerballadmin/ballColor')}</label>
                                                        <select 
                                                            value={formData.ball}
                                                    ...
                                                    <div className="form-group">
                                                        <label>{t('dhpowerballadmin/ballText')}</label>
                                                        <select 
                                                            value={formData.text}
                                                            onChange={(e) => setFormData({...formData, text: e.target.value})}
                                                        >
                                                            <option value="Odd">{t('dhpowerballadmin/ballTextOdd')}</option>
                                                            <option value="Even">{t('dhpowerballadmin/ballTextEven')}</option>
                                                            <option value="Under">{t('dhpowerballadmin/ballTextUnder')}</option>
                                                            <option value="Over">{t('dhpowerballadmin/ballTextOver')}</option>
                                                        </select>
                                                    </div>
                                                </>
                                            ) : (
                                                <div className="form-group">
                                                    <label>{t('dhpowerballadmin/combinationBalls')}</label>
                                                    {formData.balls.map((ball, index) => (
                                                        <div key={index} className="ball-input-group">
                                                            ...
                                                            >
                                                                <option value="">{t('dhpowerballadmin/placeholderBallText')}</option>
                                                                <option value="Odd">{t('dhpowerballadmin/ballTextOdd')}</option>
                                                                <option value="Even">{t('dhpowerballadmin/ballTextEven')}</option>
                                                                <option value="Under">{t('dhpowerballadmin/ballTextUnder')}</option>
                                                                <option value="Over">{t('dhpowerballadmin/ballTextOver')}</option>
                                        ...
                                        <div className="modal-footer">
                                            ...
                                            >
                                                {saving ? 'Saving...' : (editingOption ? t('dhpowerballadmin/update') : t('dhpowerballadmin/addOption'))}
                                            </button>
                                        </div>
                                    </div>
                                </div>
