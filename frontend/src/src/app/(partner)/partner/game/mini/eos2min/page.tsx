...
/**
 * EOS2MinAdminPage Component - Admin Interface for EOS2 Min Powerball Betting Options
 * 
 * This component provides an administrative interface for managing EOS2 Min Powerball betting options.
 ...
 */
export default function EOS2MinAdminPage() {
    ...
    const handleDeleteOption = async (optionId: string | number, category: 'powerball' | 'normalball') => {
        if (!confirm(t('eos2admin/confirmDelete'))) return;
    ...
    const handleResetChanges = () => {
        if (confirm(t('eos2admin/confirmReset'))) {
            // Reload from API
            loadBettingOptions();
            loadGameConfig();
        }
    ...
    return (
        <Layout>
            <Card title={t("eos2minPowerballAdminManagement")}>
                {loading && (
                    ...
                    <>
                        ...
                        <div className="level-buttons-container">
                            {Array.from({ length: 15 }, (_, index) => {
                                ...
                                if (level <= 12) {
                                    levelName = `${t('eos2admin/level')} ${level}`;
                                } else if (level === 13) {
                                    levelName = 'VIP 1';
                                } else if (level === 14) {
                                    levelName = 'VIP 2';
                        ...
                        <div className="max-betting-container" style={{ marginTop: 16, marginBottom: 16 }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                <label style={{ color: '#f3f4f6', fontWeight: 500 }}>
                                    {t('eos2admin/maxBettingValue')}:
                                </label>
                                <Input
                                    type="number"
                                    value={maxBettingValue}
                                    onChange={(e) => setMaxBettingValue(e.target.value)}
                                    placeholder={t('eos2admin/placeholderMaxBetting')}
                                    style={{ width: 120 }}
                                    size="small"
                                />
                                <Button
                        ...
                        <div className="minigame-app !min-h-auto !bg-transparent !p-0">
                            <div className="container">
                                ...
                                <div className="right-column right-col">
                                    ...
                                    <div className="pick-wrap">
                                        ...
                                        <span>{t('eos2admin/powerballCombinations')}</span>
                                        ...
                                        {powerballOptions.map((option, index) => (
                                            ...
                                            <div className="admin-controls">
                                                ...
                                                >
                                                {option.enabled ? t('eos2admin/disable') : t('eos2admin/enable')}
                                                </button>
                                                <button 
                                    ...
                                    <div className="pick-wrap">
                                        <div className="pick-header">
                                        <span>{t('eos2admin/normalballCombinations')}</span>
                                        <div className="flex items-center gap-2">
                                            <button 
                                            className="px-2 py-1 bg-blue-600 text-white text-xs rounded hover:bg-blue-700"
                            ...
                            {(showEditModal || showAddModal) && (
                                ...
                                <div className="modal-content">
                                    ...
                                    <h3>{editingOption ? t('eos2admin/editBettingOption') : t('eos2admin/addNewBettingOption')}</h3>
                                    <button 
                                    ...
                                    <div className="form-group">
                                        <label>{t('eos2admin/optionName')}</label>
                                        <input 
                                        type="text"
                                        value={formData.name}
                                        onChange={(e) => setFormData({...formData, name: e.target.value})}
                                        placeholder={t('eos2admin/placeholderOptionName')}
                                        />
                                    </div>
                                    
                                    <div className="form-group">
                                        <label>{t('eos2admin/odds')}</label>
                                        <input 
                                        type="number"
                                        step="0.01"
                                        value={formData.odds}
                                        onChange={(e) => setFormData({...formData, odds: e.target.value})}
                                        placeholder={t('eos2admin/placeholderOdds')}
                                        />
                                    </div>
                                    
                                    <div className="form-group">
                                        <label>{t('eos2admin/type')}</label>
                                        <select 
                                        value={formData.type}
                                        onChange={(e) => setFormData({...formData, type: e.target.value as 'single' | 'combination'})}
                                        >
                                        <option value="single">{t('eos2admin/singleBall')}</option>
                                        <option value="combination">{t('eos2admin/combination')}</option>
                                        </select>
                                    </div>
                                    ...
                                    {formData.type === 'single' ? (
                                        <>
                                        <div className="form-group">
                                            <label>{t('eos2admin/ballColor')}</label>
                                            <select 
                                            value={formData.ball}
                                        ...
                                        <div className="form-group">
                                            <label>{t('eos2admin/ballText')}</label>
                                            <select 
                                            value={formData.text}
                                            onChange={(e) => setFormData({...formData, text: e.target.value})}
                                            >
                                            <option value="Odd">{t('eos2admin/ballTextOdd')}</option>
                                            <option value="Even">{t('eos2admin/ballTextEven')}</option>
                                            <option value="Under">{t('eos2admin/ballTextUnder')}</option>
                                            <option value="Over">{t('eos2admin/ballTextOver')}</option>
                                            </select>
                                        </div>
                                        </>
                                    ) : (
                                        <div className="form-group">
                                        <label>{t('eos2admin/combinationBalls')}</label>
                                        {formData.balls.map((ball, index) => (
                                            ...
                                            >
                                            <option value="">{t('eos2admin/placeholderBallText')}</option>
                                            <option value="Odd">{t('eos2admin/ballTextOdd')}</option>
                                            <option value="Even">{t('eos2admin/ballTextEven')}</option>
                                            <option value="Under">{t('eos2admin/ballTextUnder')}</option>
                                            <option value="Over">{t('eos2admin/ballTextOver')}</option>
                                    ...
                                    >
                                        {saving ? 'Saving...' : (editingOption ? t('eos2admin/update') : t('eos2admin/addOption'))}
                                    </button>
                                    </div>
                                </div>
                                </div>
