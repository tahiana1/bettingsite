...
import api from "@/api";
import { partnerTransactionAPI, PartnerRollingTransaction } from "@/api/partnerTransactionAPI";
import { RxLetterCaseToggle } from "react-icons/rx";
import { ReloadOutlined } from "@ant-design/icons";

export default function RollingTransactionPage() {
  usePageTitle("Partner - Rolling Transaction");
  ...
  // Transaction data
  const [transactions, setTransactions] = useState<PartnerRollingTransaction[]>([]);
  
  // Summary data
  const [amountHeld, setAmountHeld] = useState<number>(0);
  const [rollingBalance, setRollingBalance] = useState<number>(0);
  ...
  const loadTransactions = useCallback(async () => {
    ...
    try {
      ...
      const result = await partnerTransactionAPI.getRollingTransactions({
        page: currentPage,
        perPage: pageSize,
      ...
      if (result.success) {
        ...
        setRollingBalance(result.summary.rollingBalance);
      }
    } catch (error) {
      console.error("Error loading rolling transactions:", error);
      message.error("Failed to load rolling transactions");
    } finally {
      setLoading(false);
    }
  ...
  const onAmountChange = (e: RadioChangeEvent) => {
    const currentRolling = profile?.roll || rollingBalance || 0;
    if (e.target.value === "max") {
      setAmount(currentRolling);
      form.setFieldsValue({ amount: currentRolling });
    } else {
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (profile.roll < submitAmount) {
        message.error(t("insufficientRolling") || "Insufficient rolling");
        return;
      }
      ...
      if (submitAmount < 100) {
        message.error(t("TheMinimumRollingToConvertIs100Rolling") || "The minimum rolling to convert is 100 rolling");
        return;
      }
      ...
      api("transactions/create", {
        ...
        data: {
          ...
          explation: "Rolling conversion to balance"
        }
      })
      .then((res) => {
        if (res.data?.status) {
          message.success(t("pointConversionSuccess") || "Rolling conversion request submitted successfully");
          setModalOpen(false);
          setAmount(0);
          form.resetFields();
          ...
          message.error(t("pointConversionFailed") || "Failed to submit rolling conversion request");
        }
      })
      .catch((err) => {
        console.log(err);
  ...
  const columns: TableProps<PartnerRollingTransaction>["columns"] = [
    {
      title: t("number"),
      dataIndex: "number",
      key: "number",
    ...
    },
    {
      ...
      render: (_, record) => record.user?.userid || "-",
      onHeaderCell: () => ({
        style: { backgroundColor: "#ffb3ba" }, // Light pink background
      }),
    },
    {
      title: t("rolling"),
      ...
      children: [
        {
          title: t("applicationAmount") || "Application amount",
          dataIndex: "amount",
          key: "rollingAmount",
          width: 120,
        ...
        {
          title: t("beforeProcessing") || "Before processing",
        ...
        {
          title: t("afterProcessing") || "After processing",
    ...
    {
      title: t("processedDate") || "Processing date",
      dataIndex: "approvedAt",
      key: "approvedAt",
      width: 180,
      render: (_, record) => record.approvedAt ? dayjs(record.approvedAt).format("YYYY-MM-DD HH:mm:ss") : "-",
    },
    {
      title: t("situation"),
      dataIndex: "status",
      ...
      <Card 
        title={
          <div className="flex justify-between items-center">
            <p className="text-[15px] font-bold">{t("partner/menu/rollingTransaction")}</p>
      ...
      >
        ...
        <div className="mb-4 p-4 bg-white rounded-lg border border-gray-200 flex justify-between items-center">
          <div className="flex gap-8">
            <div>
              <span className="text-gray-600">{t("amountHeld") || "Amount held"}: </span>
              <span className="font-bold text-lg">{formatNumber(amountHeld)} {t("won") || "won"}</span>
            </div>
            <div>
              <span className="text-gray-600">{t("rollingBalance") || "Rolling balance"}: </span>
              <span className="font-bold text-lg">{formatNumber(rollingBalance)} {t("won") || "won"}</span>
            </div>
          </div>
          <Button 
            type="primary" 
        ...
        {/* Rolling Application Modal */}
        <Modal
          title={t("rolling/exchangeRequest") || "Rolling Exchange Request"}
          open={modalOpen}
        ...
        >
          <Alert
            description={
              <div>
                ...
                <p>* {t("WhenYouConvertRolling,TheyWillBeMobedToYourBalance")}</p>
                <p>* {t("TheMinimumRollingToConvertIs100Rolling")}</p>
              </div>
            }
            type="info"
          ...
          >
            <Form.Item label={t("profile/balance") || "Rolling Balance"}>
              <div style={{ fontSize: 16, fontWeight: 'bold', color: '#ff4d4f' }}>
                {formatNumber(profile?.roll || rollingBalance || 0)}
              </div>
            </Form.Item>

            <Form.Item
              name="amount"
              label={t("profile/rollingAmount") || "Rolling Amount"}
              rules={[
                { required: true, message: t("profile/rollingAmount") || "Please enter rolling amount" },
                { type: 'number', min: 100, message: t("TheMinimumRollingToConvertIs100Rolling") || "Minimum is 100 rolling" }
              ]}
            >
              <Space.Compact style={{ width: '100%' }}>
                <InputNumber
                  ...
                  style={{ width: '100%' }}
                  min={0}
                  max={profile?.roll || rollingBalance || 0}
                />
                <Button onClick={() => {
                  setAmount(0);
            ...
            <Form.Item>
              <Space style={{ width: '100%', justifyContent: 'flex-end' }}>
                ...
                <Button type="primary" htmlType="submit" loading={submitting}>
                  {t("profile/applyRolling") || "Apply Rolling"}
                </Button>
              </Space>
            </Form.Item>
          </Form>
