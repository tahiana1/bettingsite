...
export default function RollingTransitionPage() {
  usePageTitle("Partner - Rolling Transition");
  ...
  const onAmountChange = (e: RadioChangeEvent) => {
    if (e.target.value == "max") {
      setAmount(pointProfile?.roll || balance || 0);
      form.setFieldsValue({ amount: pointProfile?.roll || balance || 0 });
    } else {
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (pointProfile.roll < submitAmount) {
      ...
      <Card 
        title={
          <div className="flex justify-between items-center">
            <p className="text-[15px] font-bold">{t("rollingTransition") || "Rolling Transition"}</p>
          ...
          >
            <Form.Item label={t("profile/balance") || "Rolling Balance"}>
              <div style={{ fontSize: 16, fontWeight: 'bold', color: '#ff4d4f' }}>
                {formatNumber(pointProfile?.roll || balance || 0)}
              </div>
            </Form.Item>
            ...
            >
              <Space.Compact style={{ width: '100%' }}>
                <InputNumber
                  ...
                  style={{ width: '100%' }}
                  min={0}
                  max={pointProfile?.roll || balance || 0}
                />
                <Button onClick={() => {
                  setAmount(0);
