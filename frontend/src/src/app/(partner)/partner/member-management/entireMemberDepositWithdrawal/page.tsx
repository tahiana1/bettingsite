...
interface EntireMemberDepositWithdrawal {
  id: number;
  userId: number;
  userid: string;
  nickname: string;
...
export default function EntireMemberDepositWithdrawalPage() {
  usePageTitle("Partner - Entire Member Deposit Withdrawal Page");
  const t = useTranslations();
  const f = useFormatter();
  const [loading, setLoading] = useState<boolean>(false);
  const [transactions, setTransactions] = useState<EntireMemberDepositWithdrawal[]>([]);
  ...
  const columns: TableProps<EntireMemberDepositWithdrawal>["columns"] = [
    {
      title: t("number"),
      dataIndex: "number",
      key: "number",
  ...
  const onChange: TableProps<EntireMemberDepositWithdrawal>["onChange"] = (
    pagination,
    filters,
    sorter
  ) => {
    ...
    <Layout>
      <Content className="overflow-auto h-[calc(100vh-100px)] dark:bg-black">
        <Card
          title={
            <div className="flex justify-between items-center w-full">
              <span>{t("entireMemberDepositWithdrawal") || "Entire Member Deposit Withdrawal"}</span>
        ...
        >
          ...
          <Table<EntireMemberDepositWithdrawal>
            columns={columns}
            loading={loading}
