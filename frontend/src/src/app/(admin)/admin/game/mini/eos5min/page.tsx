...
export default function EOS5MinAdminPage() {
    ...
    const handleDeleteOption = async (optionId: string | number, category: 'powerball' | 'normalball') => {
        if (!confirm(t('eos5admin/confirmDelete'))) return;
    ...
    return (
        <Layout>
            <Card title={t("eos5minPowerballAdminManagement")}>
                {loading && (
                    ...
                    <>
                        <div className="level-buttons-container">
                            {Array.from({ length: 15 }, (_, index) => {
                                ...
                                if (level <= 12) {
                                    levelName = `${t('eos5admin/level')} ${level}`;
                                } else if (level === 13) {
                                    levelName = 'VIP 1';
                                } else if (level === 14) {
                                    levelName = 'VIP 2';
                        ...
                        <div className="max-betting-container" style={{ marginTop: 16, marginBottom: 16 }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                <label style={{ color: '#f3f4f6', fontWeight: 500 }}>
                                    {t('eos5admin/maxBettingValue')}:
                                </label>
                                <Input
                                    type="number"
                                    value={maxBettingValue}
                                    onChange={(e) => setMaxBettingValue(e.target.value)}
                                    placeholder={t('eos5admin/placeholderMaxBetting')}
                                    style={{ width: 120 }}
                                    size="small"
                                />
                                <Button
                                        ...
                                        <div className="bet-sidebar">
                                            <div className="pick-wrap">
                                                <div className="pick-header">
                                                    <span>{t('eos5admin/powerballCombinations')}</span>
                                                    ...
                                                    {powerballOptions.map((option) => (
                                                        <div key={option.id} className={`pick-btn admin-pick-btn ${!option.enabled ? 'disabled' : ''}`}>
                                                            ...
                                                            <div className="admin-controls">
                                                                ...
                                                                >
                                                                    {option.enabled ? t('eos5admin/disable') : t('eos5admin/enable')}
                                                                </button>
                                                                <button 
                                            ...
                                            <div className="pick-wrap">
                                                <div className="pick-header">
                                                    <span>{t('eos5admin/normalballCombinations')}</span>
                                                    <div className="flex items-center gap-2">
                                                        <button 
                                                            className="px-2 py-1 bg-blue-600 text-white text-xs rounded hover:bg-blue-700"
                                ...
                                <div className="modal-overlay">
                                    <div className="modal-content">
                                        <div className="modal-header">
                                            <h3>{editingOption ? t('eos5admin/editBettingOption') : t('eos5admin/addNewBettingOption')}</h3>
                                            <button 
                                        ...
                                        <div className="modal-body">
                                            <div className="form-group">
                                                <label>{t('eos5admin/optionName')}</label>
                                                <input 
                                                    type="text"
                                                    value={formData.name}
                                                    onChange={(e) => setFormData({...formData, name: e.target.value})}
                                                    placeholder={t('eos5admin/placeholderOptionName')}
                                                />
                                            </div>
                                            
                                            <div className="form-group">
                                                <label>{t('eos5admin/odds')}</label>
                                                <input 
                                                    type="number"
                                                    step="0.01"
                                                    value={formData.odds}
                                                    onChange={(e) => setFormData({...formData, odds: e.target.value})}
                                                    placeholder={t('eos5admin/placeholderOdds')}
                                                />
                                            </div>
                                            
                                            <div className="form-group">
                                                <label>{t('eos5admin/type')}</label>
                                                <select 
                                                    value={formData.type}
                                                    onChange={(e) => setFormData({...formData, type: e.target.value as 'single' | 'combination'})}
                                                >
                                                    <option value="single">{t('eos5admin/singleBall')}</option>
                                                    <option value="combination">{t('eos5admin/combination')}</option>
                                                </select>
                                            </div>
                                            ...
                                            {formData.type === 'single' ? (
                                                <>
                                                    <div className="form-group">
                                                        <label>{t('eos5admin/ballColor')}</label>
                                                        <select 
                                                            value={formData.ball}
                                                    ...
                                                    <div className="form-group">
                                                        <label>{t('eos5admin/ballText')}</label>
                                                        <select 
                                                            value={formData.text}
                                                            onChange={(e) => setFormData({...formData, text: e.target.value})}
                                                        >
                                                            <option value="Odd">{t('eos5admin/ballTextOdd')}</option>
                                                            <option value="Even">{t('eos5admin/ballTextEven')}</option>
                                                            <option value="Under">{t('eos5admin/ballTextUnder')}</option>
                                                            <option value="Over">{t('eos5admin/ballTextOver')}</option>
                                                        </select>
                                                    </div>
                                                </>
                                            ) : (
                                                <div className="form-group">
                                                    <label>{t('eos5admin/combinationBalls')}</label>
                                                    {formData.balls.map((ball, index) => (
                                                        <div key={index} className="ball-input-group">
                                                            ...
                                                            >
                                                                <option value="">{t('eos5admin/placeholderBallText')}</option>
                                                                <option value="Odd">{t('eos5admin/ballTextOdd')}</option>
                                                                <option value="Even">{t('eos5admin/ballTextEven')}</option>
                                                                <option value="Under">{t('eos5admin/ballTextUnder')}</option>
                                                                <option value="Over">{t('eos5admin/ballTextOver')}</option>
                                        ...
                                        <div className="modal-footer">
                                            ...
                                            >
                                                {saving ? 'Saving...' : (editingOption ? t('eos5admin/update') : t('eos5admin/addOption'))}
                                            </button>
                                        </div>
                                    </div>
                                </div>
